...
$produit = getProduct();
$vendeur = getVendeur();
$prevente = getPreventeuser();

$showNotification = false;
$notificationMessage = '';
$notificationType = '';
if(isset($_POST['participer']) && isset($_POST['id_prevente'])){
    $res = participate([
...
<body class="min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-200 flex flex-col items-center justify-start text-gray-900">
  ...
  <script>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <?php if (empty($produit)) : ?>
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-16 text-center">
                ...
                <h3 class="text-xl font-semibold text-gray-900 mb-2">Aucune prévente disponible</h3>
                <p class="text-gray-500">Revenez bientôt pour découvrir de nouvelles offres exclusives.</p>
            </div>
        <?php else : ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[repeat(3,minmax(350px,1fr))] gap-8">
               <?php foreach ($produit as $pod) : ?>
                <?php 
                   $prev = array_filter($prevente, fn($p) => $p['id_produit'] == $pod['id_produit']);
                   $prev = reset($prev); 
                   if (!$prev) continue;    
                   $vend = array_filter($vendeur, fn($v) => $v['id_user'] == $pod['id_vendeur']);
                   $vend = reset($vend); 
                   $nbparticipation = nbparticipation($prev['id_prevente']);
               ?>
               <div class="group bg-white rounded-2xl shadow-md hover:shadow-2xl transition-all duration-300 flex flex-col overflow-hidden border border-gray-100 hover:border-blue-300">
                   <div class="relative overflow-hidden">
                       
            <div class="absolute top-4 left-4 z-20">
                <?php
                  $colorClass = ($nbparticipation >= $prev['nombre_min']) ? 'bg-green-900' : 'bg-red-800';
                ?>
                <div class="<?= $colorClass ?> text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-lg flex items-center gap-1.5">
                       ...
                       <div class="absolute top-4 right-4 z-20">
                           <span class="text-xs font-semibold px-3 py-1.5 rounded-full shadow-lg backdrop-blur-sm <?= $prev['statut'] == 'Active' ? 'bg-green-500 text-white' : 'bg-red-500 text-white' ?>">
                               <?= $prev['statut'] == 'Active' ? '✓ Active' : '✕ Annulée' ?>
                           </span>
                       </div>
                       ...
                       <?php else: ?>
                           <div class="w-full h-56 bg-gradient-to-br from-gray-100 to-gray-200 flex items-center justify-center text-gray-400 text-5xl">
                               📦
                           </div>
                       <?php endif; ?>
                   ...
                   <div class="p-6 flex-1 flex flex-col">
                       <h3 class="font-bold text-xl text-gray-900 mb-2 line-clamp-1 group-hover:text-grey-900 transition-colors">
                       ...
                       <div class="flex items-center gap-2 mb-4 pb-4 border-b border-gray-100">
                           <div class="w-8 h-8 bg-gradient-to-br from-white-100 to-purple-800 rounded-full flex items-center justify-center text-gray-800 font-semibold text-sm">
                               <?= strtoupper(substr($vend['nom_entreprise'], 0, 2)) ?>
                           </div>
    ...
    <?php if (isParticipated($prev['id_prevente'])): ?>
        <button type="button"
            class="w-full bg-gray-500 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-300 shadow-md flex items-center justify-center gap-2 cursor-not-allowed"
            disabled>
            ✅ Déjà inscrit
        </button>
    <?php else: ?>
        <button type="submit" name="participer"
            class="w-full bg-gradient-to-r from-gray-900 to-gray-900 hover:from-gray-900 hover:to-gray-500 text-white font-semibold py-3 px-4 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg flex items-center justify-center gap-2 group/btn">
            🎟️ Participer
        </button>
    <?php endif; ?>
            </form>
