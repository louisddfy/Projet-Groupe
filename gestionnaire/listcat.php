  ...
  <br>
    <div class="bg-white shadow-lg rounded-2xl p-8 w-full max-w-3xl">
        ...
        <?php if (empty($categories)) : ?>
            <p class="text-center text-gray-600">Aucune catégorie enregistrée pour le moment.</p>
        <?php else : ?>
                ...
                <table class="min-w-full border border-gray-300 rounded-lg overflow-hidden">
                    <thead class="bg-gray-800 text-white">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-sm font-semibold">Nom de la catégorie</th>
                    ...
                    <tbody class="divide-y divide-gray-200 bg-gray-50">
                        <?php foreach ($categories as $cat) : ?>
                            <tr class="hover:bg-gray-100 transition">
                                <td class="px-6 py-3"><?= htmlspecialchars($cat['id_categorie']) ?></td>
                                <td class="px-6 py-3 font-medium"><?= htmlspecialchars($cat['lib']) ?></td>
                                <td class="px-6 py-3 text-center">
                                    <div class="flex justify-center gap-3">
                                        <a href="modifcat.php?id=<?= urlencode($cat['id_categorie']) ?>"
                                           class="p-2 bg-gray-800 text-white rounded-lg hover:bg-gray-700 transition"
                                           title="Modifier">
                                            ✏️
                                        </a>

                                        <a href="suppcat.php?id=<?= urlencode($cat['id_categorie']) ?>"
